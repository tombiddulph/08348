...
SYMTABNODEPTR  symbolTable[SYMTABSIZE]; 
...
void PrintType(TERNARY_TREE t, int indent ){
	switch(t->item){
		case INTEGER_TYPE:
			printf("int ");
		break;
...
void PrintComment(char * commentBody){
	printf("/* %s */\n", commentBody);
}
	...
	static int items;
    static SYMTABNODEPTR symbolStack[128];
	SYMTABNODEPTR temporary;

void WriteCode(TERNARY_TREE t, int indent)
{

	/* these variables are marked as static to retain their values
	...
	switch(t->nodeIdentifier){
		...
		case DECLARTIONS:
			symbolStack[symbolCount++] = symbolTable[t->item];
			break;
		...
		case OUTPUT_BLOCK:
			...
			if(t->first != NULL){
				if(t->first->nodeIdentifier == FACTOR){
					printf("printf(\"");
					
					char* formatString;

					switch(symbolTable[t->first->item]->type[0]){
					case 'I': /* integer */
						formatString = "%%d";
						printf("%%d");
					break;
				...
				else if(t->first->nodeIdentifier == FACTOR_CONSTANT){
					WriteCode(t->first, indent);
					WriteCode(t->second, indent);
				}
			}
